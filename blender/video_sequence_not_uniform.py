...
class VideoSequence:
    ...
    def __init__(self,
        ...
        keys = os.listdir(self.__key_dir)
        print(keys)
        key_indexes = [int(key.split('.')[0]) for key in keys]
        key_indexes = [key_idx for key_idx in key_indexes if beg_frame <= key_idx <= end_frame]

        self.__key_indexes = key_indexes

        self.__n_seq = len(key_indexes) - 1
    ...
    def get_interval(self, i):
        key_img_index = self.get_key_img_index(i)
        try:
            next_key_img_index = self.get_key_img_index(i + 1)
        except IndexError:
            next_key_img_index = self.end_frame
        interval = next_key_img_index - key_img_index
        return interval
    ...
    def get_key_img_index(self, i):
        return self.__key_indexes[i]
